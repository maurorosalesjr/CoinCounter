...
if(amount >= .25){
  ...
  console.log(amount);
  return (`${quarters} Quarters, ` + recursiveCoinCounter((amount - (quarters * .25)).toFixed(2)));
} if(amount >= .1){
  dimes = Math.floor(amount/.1);
  console.log(amount);
  return (`${dimes} Dimes, ` + recursiveCoinCounter((amount - (dimes * .1)).toFixed(2)));
} if(amount >= .05){
  nickles = Math.floor(amount/.05);
  console.log(amount);
  return (`${nickles} Nickles, ` + recursiveCoinCounter((amount - (nickles * .05)).toFixed(2)));
} else {
